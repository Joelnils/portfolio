...
const searchUSDAFood = async (query) => {
  try {
    const searchUrl = `https://api.nal.usda.gov/fdc/v1/foods/search?query=${encodeURIComponent(query)}&api_key=${process.env.USDA_API_KEY || 'DEMO_KEY'}&pageSize=1&dataType=SR%20Legacy,Foundation`;
    const response = await makeRequest(searchUrl);
